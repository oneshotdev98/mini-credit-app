...
export default function NewApplicationWizard({ initialDefinitions }: Props) {
  const router = useRouter();
  const [tab, setTab] = useState<Tab>("build");
  const [definitions, setDefinitions] =
    useState<CustomFieldDefinition[]>(initialDefinitions);
      ...
      {tab === "upload" ? (
        <UploadParser
          onApplyToForm={(payload) => {
            setDefinitions(payload.customFieldDefinitions);
            setAppliedUploadData({
              ...payload.data,
        ...
        <ApplicationForm
          ...
          onSubmit={async (fd) => {
            ...
            const res = await fetch("/api/applications", {
              ...
              body: JSON.stringify(
              vendorDraftBodyFromFormData(fd, slugs, appliedUploadData)
            ),
            });
